...
class EWA_Importer_Support{
    ...
    public function add_column_to_importer( $options ) {
      $options['ewa-author'] = esc_html__('Author', 'everexpert-woocommerce-authors');
    	return $options;
    }
    ...
    public function add_column_to_mapping_screen( $columns ) {
      $columns[esc_html__('Author', 'everexpert-woocommerce-authors')] = 'ewa-author';
    	return $columns;
    }
    ...
    public function process_import( $object, $data ) {
      if( isset( $data['ewa-author'] ) ){
        wp_delete_object_term_relationships( $object->get_id(), 'ewa-author' );
        $authors = explode( ',', $data['ewa-author'] );
        foreach( $authors as $author ) wp_set_object_terms( $object->get_id(), $author, 'ewa-author', true );
      }
    	return $object;
    }
