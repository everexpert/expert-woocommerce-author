...
class EWA_Product_Carousel_Shortcode
{
  ...
  public static function product_carousel_shortcode($atts)
  {
    ...
    self::$atts = shortcode_atts(array(
      ...
      'autoplay'            => "false",
      'arrows'              => "false"
    ), $atts, 'ewa-product-carousel');

    //enqueue deps
    if (!wp_style_is('ewa-lib-slick'))
      wp_enqueue_style('ewa-lib-slick');
      
    if (!wp_script_is('ewa-lib-slick'))
      wp_enqueue_script('ewa-lib-slick');
  ...
  private static function products_data()
  {
    ...
    if (self::$atts['author'] != 'all') {
      $args['tax_query'] = array(
        array(
          'taxonomy' => 'ewa-author',
          'field'    => 'slug',
          'terms'    => self::$atts['author']
        )
      );
    }
